package com.itheima._24递归实现文件搜索;
...
/**
    ...
    需求：希望去D:/develop目录寻找出SecureCRT.exe文件。
...
public class FileSearchDemo {
    public static void main(String[] args) {
        // 搜索调用方法
        searchFiles(new File("D:/develop") , "SecureCRT.exe");
    }
                ...
                for (File f : files) {
                    ...
                    if(f.isFile()){
                        ...
                        if(f.getName().contains(fileName)){
                            ...
                            try {
                                // 从文件目录总定位到（.exe）然后启动软件（拓展）
                                // TODO：Runtime类就使用了单例设计模式
