    ...
    a.FileInputStream文件字节输入流。
        ...
        -- 方法：
           1.public int read():每次读取一个字节返回！读取完毕会返回-1。
                TODO：返回的是一个字节的编号，读取完毕会返回 -1。
           2.public int read(byte[] buffer):从字节输入流中读取字节到字节数组中去，
                TODO：返回读取的字节数量，没有字节可读返回-1。
        小结：
...
public class FileInputStreamDemo02 {
    public static void main(String[] args) throws Exception {
        // 需求：读取文件中的数据输出。
        // 1.创建一个文件对象
        //File srcFile = new File("Day09方法引用, Stream流，File类  , 递归 ，字节流/src/dlei02.txt");
        // 2.创建一个字节输入流管道与源文件对象接通。
        //InputStream is = new FileInputStream(srcFile);

        // 3.简化写法：直接创建一个字节输入流管道与源文件路径接通。
        InputStream is = new FileInputStream("Day09方法引用, Stream流，File类  , 递归 ，字节流/src/dlei02.txt");
