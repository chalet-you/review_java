...
public class BookSystem {
    ...
    private static void deleteBooks() {
        ...
        if (BOOK_STORE.size() == 0) {
            System.out.println("您现在根本没有任何栏目书本可以删除!");
        } else {
            queryBooks();
            // 图书馆中已经存在栏目，可以进行删除书本的业务
            ...
            while (true) {
                ...
                if (BOOK_STORE.containsKey(type)) {
                    while (true) {
                        ...
                        if (book == null) {
                            // 5.书本为空，不存在
                            System.out.println("不存在该书名的书本");
                        } else {
                            // 6.这本书存在，应该取出这个栏目的橱柜集合，然后删除该橱柜集合下的书本
                            List<Book> books = BOOK_STORE.get(type);
                            books.remove(book);
                            queryBooks();
                            System.out.println("删除成功，请看如上信息确认");
                            return;
                        }
                    }
                    ...
                    while (true) {
                        ...
                        if (book == null) {
                            ...
                            while (true) {
                                ...
                                if (newPrice.matches("\\d{1,}\\.{1}\\d{1,}")) {
                                    priceDouble = Double.parseDouble(newPrice);
                                    break; // TODO：结束循环
                                }else{
                                    System.out.println("您输入的价格格式不对，请检查");
                                }
    ...
    private static void addBook() {
        ...
        String name = SYS_SCANNER.nextLine();
        double priceDouble = 0.0;
        // TODO:使用异常处理 价格 可能输入不对，增加程序的健壮性
        while (true) {
            try {
